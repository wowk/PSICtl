...
static void init_put_bits(PutBitContext *s, uint8_t *buffer, uint32_t buffer_size)
{
    if(buffer_size < 0) {
        buffer_size = 0;
        buffer = NULL;
 ...
 */
static int32_t writtenBytes(LZWEncoderState *s){
...
int cmsLzw_initEncoder(LZWEncoderState **p, uint8_t *outbuf, uint32_t outsize)
{
    ...
    *p = (LZWEncoderState *) calloc(1, sizeof(LZWEncoderState));
    if (*p == NULL)
    {
       fprintf(stderr, "could not allocate %lu bytes for encoder state\n", sizeof(LZWEncoderState));
       return -ENOMEM;
    }
    else
    {
       //printf("%lu bytes allocated for encoder state\n", sizeof(LZWEncoderState));
    }
...
int cmsLzw_encode(LZWEncoderState *s, const uint8_t *inbuf, uint32_t insize)
{
    ...
    if(insize * 3 > (uint32_t) (s->bufsize - s->output_bytes) * 2 + round_error){
        fprintf(stderr, "size check failed, insize=%u (*3) outsize=%u (*2)\n", 
                insize, (s->bufsize - s->output_bytes));
        return -1;
    }
