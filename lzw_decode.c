...
int lzw_init_decoder(LZWDecoderState **p, uint8_t *inbuf, uint32_t inbuf_size)
{
    ...
    *p = (LZWDecoderState *)calloc(1, sizeof(LZWDecoderState));
    if (*p == NULL)
    {
       fprintf(stderr, "could not allocate %lu bytes for decoder state\n", sizeof(LZWDecoderState));
       return -ENOMEM;
    }
    else
    {
       //printf("%lu bytes allocated for decoder state\n", sizeof(LZWDecoderState));
    }
...
int32_t lzw_decode(LZWDecoderState *s, uint8_t *outbuf, uint32_t outlen)
{
    ...
    for (;;) {
        ...
        c = lzw_get_code(s);
        if (c == s->end_code) {
            //printf("got end code %d\n", c);
            break;
        } else if (c == s->clear_code) {
            //printf("got clear code %d\n", c);
            s->cursize = s->codesize + 1;
            s->curmask = mask[s->cursize];
            s->slot = s->newcodes;
            s->top_slot = 1 << s->cursize;
            ...
            if (code == s->slot && fc>=0) {
                *sp++ = fc;
                ...
                fprintf(stderr, "code %d greater than slot %d\n", code, s->slot);
                break;
            }
  ...
  the_end:
    ...
    //printf("about to return, outlen=%d l=%d\n", outlen, l);
    return outlen - l;
}
